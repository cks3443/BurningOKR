...
export class TaskBoardSwimlaneViewHelper extends TaskBoardStateColumnViewHelper implements OnDestroy {
  ...
  createKeyResultStateTaskMapList(
    keyResults: ViewKeyResult[],
    states: ViewTaskState[],
    tasks: ViewTask[],
  ): KeyResultStateTaskMap[] {
    ...
    if (copiedTasks.length >= 0) {
      map.push({
        keyResult: emptyKeyResult,
        statesWithTasks: this.createStateTaskMapList(states, tasksWithoutKeyResults),
      });
  ...
  getMovedTaskWithNewPositionDataInSwimlane(
    ...
    newTaskState: ViewTaskState, newKeyResult: ViewKeyResult,
  ): ViewTask {
