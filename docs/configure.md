...
If you only want to use Burning OKR with Docker and don't need a local development environment, follow the installation steps in the root [README](/README.md#installation).

## Manual installation

If you have installed Postgres manually on your machine, you are in the right place.
This documentation is for the local dev environment.
...
5. In your backend configuration, set provider values according to your identity provider (`keycloak` or `azureAD`).
For more information about Azure AD or Keycloak values, see [backend.env.sample](/docker/backend.env.sample).
6. Save
